<div>
    @if(($membership->kyc_status ?? 'pending') !== 'approved')
        <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg flex items-center justify-between">
            <div>Complete KYC to download your ID card.</div>
            <a href="{{ route('member.kyc-documents') }}" class="px-3 py-1.5 bg-yellow-600 text-white rounded">Go to KYC</a>
        </div>
    @endif
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex gap-3">
            @if(($membership->kyc_status ?? 'pending') === 'approved')
                <button id="downloadPng" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Download Image</button>
                <button onclick="window.print()" class="px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-800">Print</button>
            @else
                <button class="px-4 py-2 bg-gray-300 text-gray-600 rounded-lg cursor-not-allowed" disabled>Download Image</button>
                <button class="px-4 py-2 bg-gray-300 text-gray-600 rounded-lg cursor-not-allowed" disabled>Print</button>
            @endif
        </div>
    </div>
    ...
    <div id="idCard" class="max-w-2xl border rounded-2xl overflow-hidden bg-white shadow-lg">
        ...
        <div class="px-6 py-5 grid grid-cols-3 gap-6">
            <div class="col-span-2 space-y-3">
                ...
                <div>
                    ...
                    <div class="text-sm font-semibold text-gray-900">
                        ...
                        @if($membership->city), {{ $membership->city }} @endif
                        @if($membership->state), {{ $membership->state }} @endif
                    </div>
                </div>
                <div class="flex gap-2">
                ...
                <div class="pt-4">
                    ...
                    <div class="mt-2 h-8 w-40 border-t border-gray-300"></div>
                </div>
            </div>
            <div class="flex items-start justify-end">
                ...
                @else
                    <div class="w-28 h-32 border rounded bg-gray-100 flex items-center justify-center text-xs text-gray-400">No Photo</div>
                @endif
            </div>
        </div>
    ...
    <style>
        @media print {
            ...
            #idCard { box-shadow: none; border: 1px solid #e5e7eb; }
