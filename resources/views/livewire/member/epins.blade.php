        ...
        @if($myToken)
            <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-50 border rounded-lg p-4 md:col-span-2">
                    ...
                    <p class="text-sm text-gray-800">{{ $parent?->name ?? '-' }} | User Id:
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="border rounded-lg p-4">
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" wire:model="transferToToken" class="w-full border rounded px-3 py-2"
                        placeholder="Recipient Member Token">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($pins as $pin)
                        <tr>
                            ...
                            <td class="px-3 py-2 text-sm">{{ $pin->plan_name }} (₹{{ number_format($pin->plan_amount, 2) }})
                            </td>

                            <td class="px-3 py-2 text-sm">{{ $pin->status }}
                                @if($pin->usedBy)
                                    _by_
                                    {{ optional($pin->usedBy)->name }}_{{ optional($pin->usedBy)->membership_id }}
                                @endif
                            </td>
